...
% EXPAND UPON CHALLENGE FACED. INCLUDE ROADMAP AT END OF INTRO (ROADMAP OF WHAT?). FRAME THROUGH LENS OF SOLVING A PROBLEM. ARTICULATE WHY IT IS WORTH SPENDING TIME AND RESOURCES CONDUCTING THE INVESTIGATION - JUSTIFY BENEFITS OUTWEIGHING THE COST OF RESEARCH
...
% THIS IS NOT CLEARLY LINKED TO PARAGRAPHS BELOW AND SINGLE LINE PARAGRAPHS SHOULD BE AVOIDED.
...
\section{Artifact}
% LACK OF CLARITY ON SPECIFIC DESIGN OF MY INTERVENTION AND SUPPORT PROVIDED TO TEAM'S AND WHAT SPECIFIC INTERVENTION IN. DETAIL SHOULD BE SUFFICIENT FOR SOMEONE TO BE ABLE TO GO AWAY AND REPLICATE (STANDARD SCIENTIFIC RIGOUR)
...
% SOME OF THIS COULD BE INCLUDED IN THE EXPERIMENTAL DESIGN SECTION TO JUSTIFY DESIGN DECISIONS.
...
\section{Data Analysis}
    ...
    \subsection*{The Results For Each Hypothesis Test}
        ...
        \subsubsection{A team using a CD pipeline will refine their scope more often}
            Pr($>$F) of 0.0284 *. So, if we assume that there is no effect, there would be a 2.8\% probability of making these (or more extreme) observations. Thus, this is smaller than the conventional 5\% tolerance, the hypothesis was accepted. F = 5.5, $\omega^2$ = 0.16, which is a large effect size. As this hypothesis was accepted, the data was analysed to see if it fits the assumption of homoscedasticity, see Figure \ref{scopeFrequencyHomoscedasticity}, and a box and whisker diagram was generated, see Figure \ref{scopeFrequencyPlot}.\\
...
\section{Conclusion and Future Work} \label{conclusion}
    ...
    \begin{figure}[h!]
        \includegraphics[width=0.95\columnwidth]{Images/scopeFrequencyHomoscedasticity.png}
        \caption{Homoscedasticity analysis graphs for the data relating to hypothesis \ref{h_scoperefine}}
        \label{scopeFrequencyHomoscedasticity}
    \end{figure}
    \begin{figure}[h!]
        \includegraphics[width=0.95\columnwidth]{Images/scopeFrequencyPlot.png}
        \caption{A box and whisker plot showing the data relating to hypothesis \ref{h_scoperefine}}
        \label{scopeFrequencyPlot}
    \end{figure}
...
% Need to add smart actions for a pass
\subsection*{Reflective Addendum}
    ...
    In the future, a more conservative experimental design with respect to the power analysis would likely be beneficial. In the initial design, hypothesis one required the participation of 32 teams to reach statistical significance, which in hindsight was not feasible with the available resources.
...
\lstinputlisting[language =R, label=SCRAnalysis, caption =R code to perform ANOVA tests upon results after file is parsed with Python. Best fit analysis is then performed upon these tests., captionpos =b]{data_analysis/stat_analysis.r}
